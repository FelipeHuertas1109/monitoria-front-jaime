...
export class AsistenciasService {
  ...
  static async listar(query: ListarAsistenciasQuery, token: string): Promise<ListarAsistenciasResponse> {
    ...
    const url = `${this.baseUrl}/example/directivo/asistencias/?${params.toString()}`;
    ...
    if (!response.ok) {
      ...
      if (response.status === 404) throw new Error('No se encontraron asistencias');
  ...
  static async autorizar(id: number, token: string): Promise<AccionAsistenciaResponse> {
    const url = `${this.baseUrl}/example/directivo/asistencias/${id}/autorizar/`;
    const response = await fetch(url, {
      method: 'POST',
      headers: {
        'Accept': 'application/json',
  ...
  static async rechazar(id: number, token: string): Promise<AccionAsistenciaResponse> {
    const url = `${this.baseUrl}/example/directivo/asistencias/${id}/rechazar/`;
    const response = await fetch(url, {
      method: 'POST',
      headers: {
        'Accept': 'application/json',
  ...
  static async misAsistencias(query: MisAsistenciasQuery, token: string): Promise<MisAsistenciasResponse> {
    const params = new URLSearchParams();
    params.set('fecha', query.fecha);
    const url = `${this.baseUrl}/example/monitor/mis-asistencias/?${params.toString()}`;
  ...
  static async marcar(body: MarcarPresenteRequest, token: string): Promise<MarcarPresenteResponse> {
    const url = `${this.baseUrl}/example/monitor/marcar/`;
    const response = await fetch(url, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      if (response.status === 400) {
        ...
        // texto genérico o mensaje exacto del backend
